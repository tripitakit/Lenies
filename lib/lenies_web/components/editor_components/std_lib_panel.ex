defmodule LeniesWeb.EditorComponents.StdLibPanel do
  ...
  attr :open, :any, default: MapSet.new()
  ...
  def std_lib_panel(assigns) do
    ...
    <section class="std-lib-panel">
      ...
      <div :for={{category, snippets} <- @std_lib} class="std-lib-cat">
        <button
          type="button"
          class="std-lib-cat-head"
          phx-click="toggle_stdlib_cat"
          phx-value-cat={category}
          aria-expanded={MapSet.member?(@open, category)}
        >
          {category}
        </button>
        <div :if={MapSet.member?(@open, category)} class="std-lib-cat-body">
