defmodule LeniesWeb.UserAuthTest do
  use LeniesWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Lenies.Accounts
  alias Lenies.Accounts.Scope
  alias LeniesWeb.UserAuth
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, LeniesWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      LeniesWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        LeniesWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
