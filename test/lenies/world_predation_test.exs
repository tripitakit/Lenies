defmodule Lenies.WorldPredationTest do
  ...
  setup do
    ...
    :ets.insert(Lenies.WorldTestHelpers.cells(world_id), {key, %{cell | lenie_id: "P1"}})
  ...
  describe "defend" do
    test "sets defending_until on the parent record", %{world_id: world_id} do
      result = Lenies.Worlds.action(world_id, {:defend, "P1"})
      assert result == {:ok, :defending}

      [{"P1", record}] = :ets.lookup(Lenies.WorldTestHelpers.lenies(world_id), "P1")
    ...
    test "defend after multiple ticks updates relative to current tick",
    ...
    test "defend on a Lenie without :lenies record returns :no_lenie",
         %{world_id: world_id} do
      :ets.delete(Lenies.WorldTestHelpers.lenies(world_id), "P1")
      result = Lenies.Worlds.action(world_id, {:defend, "P1"})
  ...
  describe "attack" do
    setup %{world_id: world_id, handle: handle} do
      ...
      :ets.insert(Lenies.WorldTestHelpers.cells(world_id), {key, %{cell | lenie_id: "T1"}})
    ...
    test "attack on empty front cell returns :no_target",
         %{world_id: world_id, target_pid: target_pid} do
      # Move target away
      GenServer.stop(target_pid)
      Process.sleep(100)

      [{key, cell}] = :ets.lookup(Lenies.WorldTestHelpers.cells(world_id), {11, 10})
      :ets.insert(Lenies.WorldTestHelpers.cells(world_id), {key, %{cell | lenie_id: nil}})

      result = Lenies.Worlds.action(world_id, {:attack, {10, 10}, :e, "P1"})
      assert result == {:ok, :no_target}
    end

    test "attack on undefended target deals full damage",
         %{world_id: world_id, target_pid: target_pid} do
    ...
    test "attack on defended target deals halved damage and reports :defended",
         %{world_id: world_id} do
      Process.sleep(50)
      # Manually set defending_until in :lenies record for T1
      [{"T1", record}] = :ets.lookup(Lenies.WorldTestHelpers.lenies(world_id), "T1")
  ...
  describe "kill leaves carcass" do
    test "Lenie dying from :take_damage leaves cell cleared and registry cleaned",
         %{world_id: world_id, handle: handle} do
      [{key, cell}] = :ets.lookup(Lenies.WorldTestHelpers.cells(world_id), {30, 30})
      :ets.insert(Lenies.WorldTestHelpers.cells(world_id), {key, %{cell | lenie_id: "VICTIM"}})
      ...
      [{_, cell}] = :ets.lookup(Lenies.WorldTestHelpers.cells(world_id), {30, 30})
